...
class MLineEdit : public QLineEdit {
    virtual void focusInEvent(QFocusEvent* ev) override {
        ...
        case Qt::FocusReason::BacktabFocusReason:
            this->setProperty("class", "visibleFocus");
...
.visibleFocus {
    ...
    auto manualLogin = new MPushButton("Manual login", this);
    connect(manualLogin, &MPushButton::clicked, this, [this]() {
    ...
    
    {
        ...
        label = new QLabel(tr("email used during login (optional)"), this);
        layout->addWidget(label);
        layout->setStretchFactor(label, 0);
        auto emailField = new MLineEdit(this);
        layout->addWidget(emailField);
        layout->addWidget(new QLabel(tr("Value of \"oauth_token\" Cookie after login"), this));
        auto oauthTokenField = new MLineEdit(this);
        layout->addWidget(oauthTokenField);
        label = new QLabel(tr("Value of \"user_id\" Cookie after login"), this);
        layout->addWidget(label);
        layout->setStretchFactor(label, 0);
        auto userIdField = new MLineEdit(this);
        layout->addWidget(userIdField);
        layout->addStretch(1);
        layout->addWidget(new QWidget(this));
        layout->addStretch(0);
        auto login = new MPushButton("Login");
        connect(login, &MPushButton::clicked, this, [=]() {
            pAccountToken = oauthTokenField->text();
            pAccountUserId = userIdField->text();
            setAccountIdentifier(emailField->text());
            stacked->setCurrentWidget(encryptPage);
        });
    ...
    {
        encryptPage = new QWidget(this);
        QVBoxLayout *layout = new QVBoxLayout(encryptPage);
        auto label = new QLabel(tr("Please provide a password to encrypt your Google Credentials, leave blank if you don't want to provide this password every time you open the Launcher then this launcher generates a password and saves it in the config"), this);
        label->setTextInteractionFlags(Qt::TextSelectableByMouse);
        label->setWordWrap(true);
        layout->addWidget(label);
        layout->setStretchFactor(label, 0);
        layout->addWidget(new QLabel("Password", this));
        auto pwdField = new MLineEdit(this);
        pwdField->setEchoMode(MLineEdit::Password);
        layout->addWidget(pwdField);
        layout->addStretch(1);
        layout->addWidget(new QWidget(this));
        layout->addStretch(0);
        auto login = new MPushButton(tr("Save and complete Login"));
        connect(login, &MPushButton::clicked, this, [=]() {
            pEncryptToken = pwdField->text();
            accept();
        });
        layout->addWidget(login);
        stacked->addWidget(encryptPage);
    }
    ...
    if(ReadEnvFlag("USE_WEBENGINE", true)) {
        {
            ...
            layout->addWidget(label);
            auto tryagain = new MPushButton("Try again", this);
            connect(tryagain, &MPushButton::clicked, this, [this]() {
        ...
        }
        {
            ...
            layout->addWidget(label);
            auto ignoreCert = new MPushButton(tr("Go ahead and ignore this error at your own risk"));
            connect(ignoreCert, &MPushButton::clicked, this, [this]() {
        ...
        }
        {
            ...
            auto label = new QLabel(tr("Information about Security: This Launcher is not Google Play Certified and never will be supported by Google. This Launcher stores your Google Play access token accessible to any Program on your disk if you don't provide your own password on every start of the app"), this);
            label->setWordWrap(true);
            layout->addWidget(label);
            layout->setStretchFactor(label, 0);
            auto manualLogin = new MPushButton("Manual login", this);
